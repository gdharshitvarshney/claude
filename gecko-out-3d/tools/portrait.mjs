/* Close-ups for art review: pushes every gecko out, then clips tight on one head. */
import { chromium } from 'playwright';
import { mkdirSync } from 'node:fs';
const out = process.argv[2] || '/tmp/geckoart';
...
// everybody out, so the board is full of geckos
...
const clip = await p.evaluate(j => {
  ...
  const v = G.geckos[j].head.position.clone();
  v.project(cam);
